  ...
  $.fn.extend({
    cssCharts: function(opts) {
      ...
      return this.each(function() {
          ...
          else if(opts.type == "vertical_bar"){thychart.vertical_bar(this);}
...
// vertical bar chart
  vetical_bar: function(node){
    var $node = $(node);

        $node.parent().addClass("vertical_bar");

    var data = $node.attr("data-bars");
    var unit = $node.attr("data-unit");
    var width = $node.width();
    var grid = $node.attr("data-grid");
    var barHeight = $node.attr("data-height");
    var max = $node.attr("data-max");
...
//    if(!unit) unit = "%";
    ...
    $.each(data, function(i, v) {
      // first dimension
      var uls = $("<ul></ul>");
      var lis = $("<li><span></span></li>").width(width);
    ...
    for(var i = 0; i <= 10; i++) {
      ...
      if(i % 2 === 0){
        var line = $("<hr/>").css({left: toPerc+"%"}).attr("data-x", toUnit + unit);
        $node.parent().find(".grid").append(line);
      }
    }
