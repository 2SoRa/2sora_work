<template>
  <div class="schedule">
    <div class="gnb">
      <button class="back" @click="closeSchedule()"></button>
      <div v-if="scheduleList.todayStudyYn == 'N'" class="gnb-left">
        <img src="@/assets/img/launcher/schedule/ttable_autonomy@2x.webp" alt="자율학습모드">
      </div>
      <div class="gnb-title">
        <img src="@/assets/img/launcher/schedule/ttable_sub_navi_ico_l.png" alt="deco">
        <p v-if="getParentAuthDiv == 'P'">학습 시간표 관리</p>
        <p v-else>학습 시간표</p>
        <img src="@/assets/img/launcher/schedule/ttable_sub_navi_ico_r.png" alt="deco">
      </div>
      <div class="gnb-right all-study-list">
        <span>
          <router-link :to="{ name: 'ScheduleStatus'}">
            <img src="@/assets/img/launcher/schedule/ttable_btn_learningview.png" alt="learningview">
          </router-link>
        </span>
        <button class="btn" @click="closeSchedule()">
          <img src="@/assets/img/launcher/mypage/mp_navi_btn_close_b.png" alt="close">
        </button>
      </div>

    </div>
    <div class="study-plan">
      <div class="schedule-area">
        <div class="calendar-area">
          <FullCalendar :options="calendarOptions"/>
          <div class="progress-area">
            <progress id="progress" :value="scheduleList.actCnt" min="0" :max="scheduleList.planCnt"></progress>
            <p><span>{{ scheduleList.actCnt }}</span>/{{ scheduleList.planCnt }}</p>
          </div>
          <p class="weekly">{{ weekOfMonthTitle }}</p>
        </div>
        <div class="schedule-bottom"></div>
      </div>
    </div>
  </div>

  <AuthModal :propsData="{authShowModal}" @closePopup="closeAuthPopup"></AuthModal>
  <!-- TODO: AuthModal 인증 후 시간표 로딩 필요 -->
  <DetailModal ref="scriptDetail"
               :propsData="{detailShowModal, student_id, fullDate, year, month, date, dayText, diffDate, diffPrevDate, isThisMonthPrev, isThisMonthNext}"
               @closePopup="closeDetailPopup" @getScheduleList="getScheduleList" @showPrevDetail="showPrevDetail"
               @showNextDetail="showNextDetail"></DetailModal>
  <confirmModal :propsData="{confirmShowModal, message}" @closePopup="closeConfirmPopup"></confirmModal>
</template>

<script>
import {mapGetters, mapMutations} from "vuex";
import '@fullcalendar/core/vdom' // solves problem with Vite
import FullCalendar from '@fullcalendar/vue3'
import dayGridPlugin from '@fullcalendar/daygrid'
import interactionPlugin from '@fullcalendar/interaction'
import moment from "moment";

import AuthModal from './comp-schedule-popup/schedule-parent-auth-popup'
import DetailModal from './comp-schedule-popup/schedule-detail-popup'
import confirmModal from './comp-schedule-popup/comfirm-popup'

import learningFactory from '@/api/learning-factory';

const schedule = learningFactory.get('scheduleCode');

export default {
  components: {
    FullCalendar, // make the <FullCalendar> tag available
    AuthModal,
    DetailModal,
    confirmModal,
  },
  data() {
    return {
      authShowModal: false,
      detailShowModal: false,
      completeShowModal: false,
      confirmShowModal: false,
      message: '',

      isBackMy: false,
      diffDate: true, // true면 현재보다 훗날
      diffPrevDate: true, // true면 현재포함 전날
      isThisMonthPrev: true,
      isThisMonthNext: true,

      weekOfMonthTitle: '',
      year: '',
      month: '',
      date: '',
      day: '',
      dayText: '',
      student_id: '',
      fullDate: '',

      scheduleList: [],
      getEventData: [],
      tempList: [],
      getDetailData: [],

      calendarOptions: {
        plugins: [dayGridPlugin, interactionPlugin],
        // locale: 'en',
        initialView: 'dayGridMonth',
        // clickToCreate: false,
        firstDay: 1,
        // 이벤트 3개 이상일때 더보기
        dayMaxEvents: 3,
        // 달력 마지막 줄 삭제
        fixedWeekCount: false,
        // 달력칸 높이 자동 조정
        contentHeight: 'auto',
        headerToolbar: { //헤더 툴바 커스텀
          left: 'processSchedule',
          // TODO: .fc-next-button & .fc-prev.button 에 disable 추가시 비활성화로 버튼 이미지 변경됨
          center: 'prev title next',
          // right: 'modiSchedule manageSchedule'
          right: 'manageSchedule' /* TODO: 학습계획 오픈시 '학습계획 오픈시 제거' 텍스트 검색으로 calendar.css에서 visibility: hidden; 삭제 */
        },
        // 커스텀 버튼
        customButtons: {
          processSchedule: {
            text: '',
            click: function () {
              // alert('이번달 학습현황 버튼 클릭');
            }
          },
          //TODO: 클릭시 담임선생님 쪽지로 이동
          modiSchedule: {
            text: '',
            click: function () {
              alert('clicked the custom button!');
            }
          },
          //TODO: 클릭시 학부모 인증 후 시간표 관리 페이지로 이동 / 이미 인증한 경우 버튼변경 / class = "end-schedule" 추가
          manageSchedule: {
            text: '',
            click: this.openAuthPopup,
          }
        },
        businessHours: [{
          //daysOfWeek:[1,2,3,4,5] //요일 별로 표시하고 싶을 때 1~5는 월~금
        }],
        events: [],
        // hiddenDays: [0, 6], // TODO: 주말 달력에서 나오지 않게 하기로 확정되면 주석 해제 필요
        titleFormat: this.setTitleDate,
        // 이벤트 팝업 타이틀(날짜)변경
        dayPopoverFormat: function (date) {
          const year = date.date.year;
          const month = date.date.month + 1;
          const day = date.date.day;

          return year + "년 " + month + "월 " + day + "일";
        },
        dayHeaderContent: function (date) {
          let weekList = ["일", "월", "화", "수", "목", "금", "토"];
          return weekList[date.dow];
        },
        eventChange: function (arg) {
          //allDay true로 바꾸면 end가 없어서 만듬
          if (arg.event.end == null) {
            var end = new Date();
            end.setDate(arg.event.start.getDate() + 1);
            arg.event.setEnd(end);
          }
        },
        dateClick: this.openDetailPopup,
        datesSet: this.getDateButton,
        eventOrder: true,// 이벤트 순서를 값 들어오는 순서대로
        moreLinkContent: function (args) {
          return args.num;
        },
      },
    }
  },
  watch: {
    'year'() {
      this.getScheduleList();
    },
    'month'() {
      this.getScheduleList();
    },
    'getParentAuthDiv'() {
      this.checkParentAuthDiv();
    },
  },
  created() {
    this.student_id = this.userInfo.info.studentId;

    this.setWeekOfMonthTitle();
    this.getParentSmsInfo();
    this.checkBackPage();

    setTimeout(() => {
      this.getScheduleList();
    }, 0);
  },
  mounted() {
    this.checkParentAuthDiv();
  },
  computed: {
    ...mapGetters('scheduleStorage', ['getParentAuthDiv']),
    ...mapGetters(['userInfo']),
  },
  methods: {
    ...mapMutations('scheduleStorage', ['storeDetailList', 'storeParentSmsInfo', 'storeParentAuthDiv']),

    // 마이페이지에서 왔는지 메인에서 왔는지 체크
    checkBackPage() {
      const urlParams = new URL(location.href).searchParams;
      const isMy = urlParams.get('backDiv');
      if (isMy == 'Y') {
        this.isBackMy = true;
      } else {
        this.isBackMy = false;
      }
    },
    checkParentAuthDiv() {
      const authBtn = document.querySelector('.fc-manageSchedule-button');
      if (this.getParentAuthDiv == 'P') {
        authBtn.classList.add('end-schedule');
      } else {
        authBtn.classList.remove('end-schedule');
      }
    },
    getParentSmsInfo() {
      const data = {
        student_id: this.student_id,
      }

      const list = schedule.loadParentSmsInfo(data);
      list.then((res) => {
        if (res.status == 200) {
          this.storeParentSmsInfo(res.data.data.mdnNo);
        }
      }).catch((error) => {
        console.log(error)
      });
    },
    getScheduleList() {
      const data = {
        student_id: this.student_id, // this.student_id, 5259 = 상담뱃지 나오는 테스트 학생아이디
        search_year: this.year,
        search_month: this.month
      }

      const list = schedule.getList(data);
      list.then((res) => {
        if (res.status == 200) {
          this.scheduleList = res.data.data;
          this.getEventData = res.data.data.result;

          // this.getAttendEventData = res.data.data.attendList; // TODO: 출석관련 필요시 주석 해제
          this.setDataList();

          let barMax = document.querySelector('.study-plan');
          if (this.scheduleList.actCnt == this.scheduleList.planCnt) {
            barMax.classList.add('max');
          } else {
            barMax.classList.remove('max');
          }

        }
      }).catch((error) => {
        console.log(error)
      });
    },
    setDataList() {
      this.tempList = [];

      for (let i = 0; i < this.getEventData.length; i++) {
        this.tempList.push({});
        for (let j = 0; j < this.tempList.length; j++) {
          if (i == j) {
            this.tempList[j].title = `${this.getEventData[i].subj_nm}>${this.getEventData[i].prog_nm}`;
            this.tempList[j].date = this.getEventData[i].study_plan_dt;
            this.tempList[j].active = this.getEventData[i].act_yn;

            if (this.getEventData[i].subj_nm == '국어') {
              this.tempList[j].className = 'korean';
              if (this.tempList[j].active != 'N') {
                this.tempList[j].className = 'korean study';
              }
            } else if (this.getEventData[i].subj_nm == '수학') {
              this.tempList[j].className = 'math';
              if (this.tempList[j].active != 'N') {
                this.tempList[j].className = 'math study';
              }
            } else if (this.getEventData[i].subj_nm == '영어') {
              this.tempList[j].className = 'english';
              if (this.tempList[j].active != 'N') {
                this.tempList[j].className = 'english study';
              }
            } else if (this.getEventData[i].subj_nm == '슈퍼박스') {
              this.tempList[j].className = 'superbox';
              if (this.tempList[j].active != 'N') {
                this.tempList[j].className = 'superbox study';
              }
            } else if (this.getEventData[i].subj_nm == '특별학습') {
              this.tempList[j].className = 'special';
              if (this.tempList[j].active != 'N') {
                this.tempList[j].className = 'special study';
              }
            } else {
              this.tempList[j].className = 'other';

              if (this.tempList[j].active != 'N') {
                this.tempList[j].className = 'other study';
              }
            }
          }
        }
      }

      this.calendarOptions.events = this.tempList;
    },
    getDateButton() {
      this.checkMonth(); // 학습계획은 현재 기준으로 다음달까지만 잡을 수 있는 처리
      this.showConsultImg(); // 화상뱃지 표시
    },
    setTitleDate(date) {
      const year = date.date.year;
      const month = String(date.date.month + 1).padStart(2, '0');

      this.year = year;
      this.month = month;

      return year + "년 " + month + "월";
    },
    checkMonth() {
      // 학습계획은 현재 기준으로 다음달까지만 잡을 수 있는 처리
      const moment = require('moment');
      let currentYear = moment().year();
      let currentMonth = moment().month() + 1;

      let diff = (currentMonth - this.month) + (12 * (currentYear - this.year));

      let nextBtn = document.querySelector('.fc-next-button');
      if (diff <= -1) {
        nextBtn.classList.add('disable');
      } else {
        nextBtn.classList.remove('disable');
      }

      let prevBtn = document.querySelector('.fc-prev-button');
      if (diff >= 12) {
        prevBtn.classList.add('disable');
      } else {
        prevBtn.classList.remove('disable');
      }
    },
    showConsultImg() {
      // 화상뱃지 표기하기
      const moment = require('moment');

      // 상담뱃지 전부 삭제 후 이벤트가 있는 곳에만 추가
      const counselBtn = document.querySelectorAll('.fc-daygrid-day-counseling.counsel');
      counselBtn.forEach((el) => {
        el.remove();
      });

      // 각 날짜와 화상 날짜 값을 비교해서 같은 곳에 화상뱃지 달기
      let eachDateWrap = document.querySelectorAll('.fc-daygrid-day');

      eachDateWrap.forEach((el) => {
        let thisDateTag = el;
        const day = moment(thisDateTag.getAttribute('data-date'));
        // let eachDate = String(day.date()).padStart(2, '0');

        const weekNumber = this.getWeekNumber(day.toDate());

        // console.log(thisDateTag.getAttribute('data-date'));
        // console.log(weekNumber);

        let isWeek = false;
        let isDayweek = false;

        if (this.scheduleList.mnglist != undefined) {

          const mngWeek = this.scheduleList.mnglist.mng_week;
          const mngDayweek = this.scheduleList.mnglist.mng_dayweek;
          if (mngWeek != null && mngWeek != undefined) {
            // 주차에 해당하는 경우
            if (mngWeek.indexOf(String(weekNumber)) >= 0) {
              isWeek = true;
            }
          }
          if (mngDayweek != null && mngDayweek != undefined) {
            // 주차에 해당하는 경우
            if (this.getDayString(mngDayweek) === day.day()) {
              isDayweek = true;
            }
          }

          if (isWeek && isDayweek) {
            let dailyBtn = document.createElement("div");
            dailyBtn.setAttribute('class', 'fc-daygrid-day-counseling counsel');
            thisDateTag.querySelector('.fc-daygrid-day-top').appendChild(dailyBtn);
            dailyBtn.setAttribute('data-date', thisDateTag.getAttribute('data-date'));
          }
        }

        // if (String(moment(thisDateTag.getAttribute('data-date')).month() + 1).padStart(2, '0') == this.month) {
        //   this.scheduleList.videolist?.forEach((el) => {
        //     let dailyBtn = document.createElement("div");
        //     dailyBtn.setAttribute('class', 'fc-daygrid-day-counseling counsel');
        //
        //     if (eachDate == el.check_dt) {
        //       thisDateTag.querySelector('.fc-daygrid-day-top').appendChild(dailyBtn);
        //       dailyBtn.setAttribute('data-date', el);
        //     }
        //   });
        // }
      });
    },
    showPrevDetail() {
      const moment = require('moment');
      this.fullDate = moment(this.fullDate).subtract('1', 'd').locale('ko').format('YYYY-MM-DD');

      this.openDetailPopup(this.fullDate, 'arrow');
    },
    showNextDetail() {
      const moment = require('moment');
      this.fullDate = moment(this.fullDate).add('1', 'd').locale('ko').format('YYYY-MM-DD');

      this.openDetailPopup(this.fullDate, 'arrow');
    },

    // 팝업 클릭시
    clickPopBtn() {
      document.querySelector('body').style.overflow = 'hidden';
    },
    // 학부모 인증 팝업 열기
    openAuthPopup() {
      if (this.getParentAuthDiv == 'P') {
        this.message = '시간표 관리 모드를 종료하겠습니까?';
        this.openConfirmPopup();
        this.storeParentAuthDiv('NP');
        const authBtn = document.querySelector('.fc-manageSchedule-button');
        authBtn.classList.add('end-schedule');
      } else {
        this.authShowModal = true;
        this.clickPopBtn();
      }
    },
    // 학부모 인증 팝업 닫기
    closeAuthPopup() {
      this.authShowModal = false;

      document.querySelector('body').style.overflow = 'auto';
    },
    // 상세보기 팝업 열기
    openDetailPopup(arg, string) {

      const moment = require('moment');
      let today = moment();

      if (string == 'arrow') {
        this.fullDate = arg;
        this.date = moment(this.fullDate).format('D');
        this.day = moment(this.fullDate).format('d');
      } else {
        this.fullDate = arg.dateStr;
        this.date = arg.date.getDate();
        this.day = arg.date.getDay();
      }

      const diffDate = today.startOf('day').diff(this.fullDate, 'days'); // 현재보다 전날이면 양수, 현재보다 훗날이면 음수
      if (diffDate > 0) { // 현재보다 전날이면 계획추가 버튼 삭제
        this.diffDate = false;
      } else {
        this.diffDate = true;
      }

      if (diffDate >= 0) { // 현재포함 전날이면 보기버튼 추가
        this.diffPrevDate = true;
      } else {
        this.diffPrevDate = false;
      }

      if (this.day == 0) {
        this.dayText = '일';
      } else if (this.day == 1) {
        this.dayText = '월';
      } else if (this.day == 2) {
        this.dayText = '화';
      } else if (this.day == 3) {
        this.dayText = '수';
      } else if (this.day == 4) {
        this.dayText = '목';
      } else if (this.day == 5) {
        this.dayText = '금';
      } else if (this.day == 6) {
        this.dayText = '토';
      }

      if (this.month != moment(this.fullDate).format('M').padStart(2, '0')) {
        return;
      }

      // 화살표 막기
      if (this.fullDate == moment(this.fullDate).startOf('month').format('YYYY-MM-DD')) {
        this.isThisMonthPrev = false;
      } else {
        this.isThisMonthPrev = true;
      }

      // 화살표 막기
      if (this.fullDate == moment(this.fullDate).endOf('month').format('YYYY-MM-DD')) {
        this.isThisMonthNext = false;
      } else {
        this.isThisMonthNext = true;
      }

      this.clickPopBtn();
      this.getDetailList();

      this.detailShowModal = true;
      this.$refs.scriptDetail.checkParentAuthTime();
    },
    // 상세보기 팝업 닫기
    closeDetailPopup() {
      this.detailShowModal = false;
      this.getDetailData = [];
      document.querySelector('body').style.overflow = 'auto';
    },
    // 팝업
    // 알럿팝업 열기
    openConfirmPopup() {
      this.confirmShowModal = true;
      document.querySelector('body').style.overflow = 'hidden';
    },
    // 알럿팝업 닫기
    closeConfirmPopup() {
      this.confirmShowModal = false;
      document.querySelector('body').style.overflow = 'auto';
    },

    // 상세보기 내용 가져오기
    getDetailList() {
      const data = {
        student_id: this.student_id,
        search_date: this.fullDate,
      }

      const list = schedule.getDetailConts(data);
      list.then((res) => {
        if (res.status == 200) {
          this.getDetailData = res.data.data;
          this.storeDetailList(this.getDetailData);
        }
      }).catch((error) => {
        console.log(error)
      });
    },
    setWeekOfMonthTitle() {
      const weekOfMonth = (m) => m.week() - moment(m).startOf('month').week() + 1;
      const nowDate = moment().utc(true);
      const week = weekOfMonth(nowDate);

      const monday = moment().day(1).format('MM.DD');
      const sunday = moment().day(7).format('MM.DD');

      this.weekOfMonthTitle = nowDate.format('MM월 ') + week + '주차' + '(' + monday + '~' + sunday + ') 학습 중';
    },
    closeSchedule() {
      if (this.isBackMy) {
        document.location.href = "/mypage.html"
      } else {
        // eslint-disable-next-line
        goGrowvAppPage("main");
      }
    },
    /**
     * 주차 계산
     * @param dateFrom
     * @returns {number}
     */
    getWeekNumber(dateFrom) {
      const localDate = new Date(dateFrom);
      var d = new Date(+localDate);
      if (isNaN(d)) return;
      // Move to previous Monday
      d.setDate(d.getDate() - d.getDay() + 1);
      // Week number is ceil date/7
      return Math.ceil(d.getDate()/7);
    },
    /**
     * 요일 문자를 숫자로 변경
     * @param day
     * @returns {number}
     */
    getDayString(day) {
      switch (day) {
        case '월':
          return 1;
        case '화':
          return 2;
        case '수':
          return 3;
        case '목':
          return 4;
        case '금':
          return 5;
      }
    },
  }
}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
/*TODO: 상담뱃지 일단 제거*/
.fc-daygrid-day-top .fc-daygrid-day-counseling {
  display: none;
}
</style>