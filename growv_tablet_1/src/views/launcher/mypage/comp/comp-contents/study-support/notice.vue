    ...
    <div class="table-wrap">
      <table>
        ...
        <tr v-for="(item, index) in tableData" :key="item">
          ...
          <td class="div">{{ item.noticeTypeNm }}</td>
          <td class="detail"><router-link :to="{ name: 'noticeView', params: { id: item.id }, query:{ name: readingClubQuery }  }">{{ item.noticeTitle }} </router-link><span class="new" v-if="Math.abs((new Date(item.regDtm).getTime() - new Date(setCurrentDate).getTime())/ (1000 * 60 * 60 * 24)) < 7"></span></td>
...
export default {
  ...
  methods: {
    ...
    getData(data, isAdd) {
      ...
      const list = noticeCode.getList(data);
      list.then((res) => {
        if (res.status == 200) {
          if (!isAdd) { // 처음 붙일 리스트
    ...
    getList() {
      const data = {
        page : 1,
        pageSize : this.limit,
        noticeTypeCd : this.formData.selectedObject,
        noticeTitle : this.formData.textTitle,
        noticeProdDiv : this.readingClubQuery ? 'R' : 'V'
      }
