            ...
            if(response.data.code == "200") {
                ...
                if (studentList.length > 0) {
                    ...
                    $.each(studentList, function (index, item) {
                        ...
                        if(!common.isNull(item.samCheck) && item.samCheck == true){
                            html +='<button type="button" onclick="fnGetSamCouponList(null, \''+studentId+'\');">교보문고 sam 이용권</button>';
                        }
                        html +='</div>';
                        html +='</div>';
                    });
...
function fnUpdateParentPwCheck(){
    ...
    if(!common.isNull(jwtTokcen.getItem(ACCESS_TOKEN_NAME)) && !common.isNull(jwtTokcen.getItem(EXPIRED_TIME_NAME)) && !common.isNull(jwtTokcen.getItem(REFRESH_TOKEN_NAME))) {
        if(!common.isNull(jwtTokcen.getItem("vuex"))){
            ...
            if(!common.isNull(userInfo.parentId) && common.isNull(parentEmail)){
